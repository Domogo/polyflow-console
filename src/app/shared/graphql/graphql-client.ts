...
import { GetPortfolioQuery } from './queries/get-portfolio';
@Injectable({
    providedIn: 'root'
})
export class GQLClient {
    ...
    findEventsById(
        vars: FindEventsByIdQueryVars
    ): Observable<Event> {
        return this._gqlClient$.pipe(
            switchMap(client => client.requestSafe<FindEventsByIdQueryResult, FindEventsByIdQueryVars>(FindEventByIdQuery, vars)),
            switchMap(response => {
                if (response.ok) {
                    return of(response.data.findEventById)
                } else {
                    return throwError(() => response.error)
                }
            })
    ...
    getPortfolio(
        vars: GetPortfolioQueryVars
    ): Observable<WalletPortfolioResponse> {
        return this._gqlClient$.pipe(
            switchMap(client => 
                client.requestSafe<GetPortfolioQueryResult, GetPortfolioQueryVars>(
                    GetPortfolioQuery, vars
                )
            ),
            switchMap(response => {
                if(response.ok) {
                    return of(response.data.getPortfolio)
                } else {
                    return throwError(() => response.error)
                }
            })
    ...
    getUserWalletAndTransactionStats(
        vars: StatsVars
    ): Observable<UsersWalletAndTransactionsInfo[]> {
        return this._gqlClient$.pipe(
            switchMap(client => 
                client.requestSafe<GetUserWalletAndTransactionStatsQueryResult, StatsVars>(
                    GetUserWalletAndTransactionStatsQuery, vars
                )
            ),
            switchMap(response => {
                if(response.ok) {
                    return of(response.data.getUserWalletAndTransactionStats)
                } else {
                    return throwError(() => response.error)
                }
            })
